...
class MySpider(BaseSpider):
    ...
    cursor.execute("select distinct link from review_demo where noofreview not like '0%'")
    
    # a='http://www.drugs.com/comments/isotretinoin/accutane-for-acne.html'
    start_urls=[]
    for i in cursor:
        link=str(i).replace("(","").replace(")","").replace(",","").replace("'","")
        for i in range(1,20):
    ...
    def parse(self,response):
        ...
        result = zip(name,varReviewID,drugname,[x.encode('ascii', 'ignore') for x in varReview])
